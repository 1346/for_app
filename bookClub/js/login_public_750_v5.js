...
$("head").append('<link rel="stylesheet" type="text/css" href="https://htxq.oss-cn-beijing.aliyuncs.com/test/login_public_750_v3.css">');
		...
		"<section class='login2'>" +
			"<section class='login_div' style='position:" + orFixed + "'  id='login_div'>" +
				"<header id='header' class='mui-bar mui-bar-nav'>"+
					...
					"<a class='mui-action-back mui-icon mui-icon-closeempty mui-pull-right' onclick='deleLogin()'></a>"+
				"</header>" +
			...
			"<section class='login_div' style='display:none;position:" + orFixed + "' id='login_div2'>" +
				"<header id='header' class='mui-bar mui-bar-nav'>"+
					"<a class='mui-action-back mui-icon mui-icon-arrowleft mui-pull-left'id='mui-icon-arrowleft' onclick='loginBack()'></a>"+
					"<h1 class='mui-title'>请输入验证码</h1>"+
					"<a class='mui-action-back mui-icon mui-icon-closeempty mui-pull-right' onclick='deleLogin()'></a>"+
				"</header>" +
		    	...
		    	if ( json.code == "000000" ) {
			    	...
			    	try {
                        console.log(json.data);
					   	...
					   	// localStorage.setItem('ExistAddress',json.data.existDefaultAddress);
					} catch (e) {
					    // alert("请关闭无痕浏览重新购买")
					}
	...
	$.ajax({
	    ...
	    success: function (json) {
			wx.config({
			    ...
			    jsApiList: ["onMenuShareTimeline","onMenuShareAppMessage","onMenuShareQQ","onMenuShareQZone","hideOptionMenu","showOptionMenu","chooseWXPay"]
